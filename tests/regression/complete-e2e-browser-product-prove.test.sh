...
# live chrome dump-dom proves product page Power Claude + install CTA.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PROVE="$ROOT/scripts/complete-e2e/browser_product_prove.py"
RUNTIME="$ROOT/configs/complete-e2e/runtime.json"
[[ -f "$PROVE" && -f "$RUNTIME" ]] || {
  echo "FAIL browser-product-prove: missing script or runtime.json" >&2
  exit 1
}

tmp="$(mktemp -d "${TMPDIR:-/tmp}/pc-ce2e-browser-prove-XXXXXX")"
trap 'rm -rf "$tmp"' EXIT
...
assert any(c.get("id") == "browser_product_page" and c.get("ok") is False for c in cases), cases
print("PASS  browser_product_prove blocked_environment on missing chrome")
PY

# 2) Live chrome dump-dom must prove product page (real DOM, not urllib).
set +e
...
assert stdout == receipt
assert rc == 0, f"live browser prove exited {rc}; stderr=\n{stderr[-2000:]}"
...
detail = next(c.get("detail") for c in cases if c.get("id") == "browser_product_page")
assert "Power Claude" in detail or "install CTA" in detail, detail
ids = {a.get("id") for a in (runtime.get("adapters") or [])}
assert "power-claude-browser-product-prove" in ids, ids
print("PASS  browser_product_prove live DOM")
PY

echo "PASS complete-e2e-browser-product-prove"
