...
# live chrome dump-dom proves Open VSX listing Power Claude + Install/Download.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PROVE="$ROOT/scripts/complete-e2e/browser_openvsx_prove.py"
RUNTIME="$ROOT/configs/complete-e2e/runtime.json"
[[ -f "$PROVE" && -f "$RUNTIME" ]] || {
  echo "FAIL browser-openvsx-prove: missing script or runtime.json" >&2
  exit 1
}

tmp="$(mktemp -d "${TMPDIR:-/tmp}/pc-ce2e-browser-openvsx-prove-XXXXXX")"
trap 'rm -rf "$tmp"' EXIT
...
assert any(c.get("id") == "browser_openvsx_page" and c.get("ok") is False for c in cases), cases
print("PASS  browser_openvsx_prove blocked_environment on missing chrome")
PY
...
assert stdout == receipt
assert rc == 0, f"live browser openvsx prove exited {rc}; stderr=\n{stderr[-2000:]}"
...
cases = (receipt.get("behavior") or {}).get("cases") or []
assert any(c.get("id") == "browser_openvsx_page" and c.get("ok") is True for c in cases), cases
detail = next(c.get("detail") for c in cases if c.get("id") == "browser_openvsx_page")
assert "Power Claude" in detail or "Open VSX" in detail or "listing signals" in detail, detail
ids = {a.get("id") for a in (runtime.get("adapters") or [])}
assert "power-claude-browser-openvsx-prove" in ids, ids
print("PASS  browser_openvsx_prove live DOM")
PY

echo "PASS complete-e2e-browser-openvsx-prove"
