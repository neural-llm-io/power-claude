...
assert isinstance(cases, list) and len(cases) >= 37, cases
...
print(
    ...
    "readme_media+consumer_complete_e2e+packed_cli_help+packed_cli_help_cluster+packed_dual_bin_version+packed_bin_payload+packed_media_walkthrough+packed_pkg_meta+packed_engine_enc+packed_media_marketplace_assets+packed_icon_extension_js+packed_files_contract+packed_uninstall_hook+packed_brand_assets+"
    "packed_artifact_sha256+packed_registry_integrity+packed_prices_contract+packed_feature_matrix_stamps+packed_jq_platform_sha+registry_dist_extras+registry_dist_file_count+registry_metadata+registry_tarball_meta+registry_listing_meta+marketplace_api+marketplace_api_listing_meta+marketplace_api_short_description+marketplace_vsix_download+marketplace_asset_heads+marketplace_listing_links+"
