...
assert isinstance(cases, list) and len(cases) >= 51, cases
ids = {c.get("id") for c in cases}
assert "readme_media" in ids and "consumer_complete_e2e" in ids and "browser_product_page" in ids and "browser_pricing_page" in ids and "browser_openvsx_page" in ids, ids
for rid in (
    "packed_cli_help",
...
assert "power-claude-browser-openvsx-prove" in ids_rt, ids_rt
...
print(
    ...
    "readme_media+consumer_complete_e2e+browser_product_page+browser_pricing_page+browser_openvsx_page+packed_cli_help+packed_cli_help_cluster+packed_cli_help_documents+packed_dual_bin_version+packed_dual_bin_paths+packed_cli_emergency_on_help+packed_cli_halt_emergency_off_help+packed_bin_payload+packed_media_walkthrough+packed_pkg_meta+packed_engine_enc+packed_media_marketplace_assets+packed_icon_extension_js+packed_files_contract+packed_uninstall_hook+packed_brand_assets+"
