...
CASE_MARKERS: dict[str, tuple[str, ...]] = {
    ...
    # Includes doctor/resume --help mentions emergency (prefix-shadowed by shallow cluster markers;
    # without elevating, cluster+docs could greenwash without the emergency document lines).
    ...
    # Open VSX API namespace/name already proven by consumer.py (exact PASS substring; NOT open_vsx_listing_meta).
    "open_vsx_api_namespace_name": (
        "PASS  open-vsx api namespace/name ",
    ),
    ...
    # package.json version already proven by consumer.py (exact PASS substring; NOT packed_dual_bin_version / NOT packed_pkg_meta).
    "package_json_version": (
        "PASS  package.json version ",
    ),
    # Packed CHANGELOG present + CHANGELOG head <= published already proven by consumer.py (require BOTH; NOT packed_pkg_meta / NOT package_json_version).
    "packed_changelog_present": (
        "PASS  CHANGELOG head ",
        "PASS  packed CHANGELOG ",
    ),
}
