...
open_vsx_vsix_download, open_vsx_asset_heads, open_vsx_icon_integrity, open_vsx_download_count, open_vsx_api_namespace_name, open_vsx_listing_meta, product_site_links, pricing_page_links, product_pricing_site_bodies, package_json_version, packed_changelog_present, browser_product_page).
...
def _validate_receipt(receipt: dict[str, Any]) -> str | None:
    ...
    for need in ("readme_media", "consumer_complete_e2e", "browser_product_page", *RICH_CASE_IDS):
        c = by_id.get(need)
        if not c:
